// Convert topojson countries-110m → SVG with data-iso per country path.
// Equirectangular projection in viewBox 1000x500.
const fs = require('fs');
const path = require('path');

const ROOT = path.resolve(__dirname, '..', 'public', 'assets', 'macro-maps');
const topo = JSON.parse(fs.readFileSync(path.join(ROOT, 'world.topo.json'), 'utf8'));

// Decode topojson arcs (delta-decoded)
...
// Equirectangular: lon [-180, 180] -> x [0, 1000]; lat [85, -60] -> y [0, 500]
...
function ringToPath(arcIdxs) {
  ...
  for (const idx of arcIdxs) {
    ...
    for (let i = 0; i < a.length; i++) {
      if (first && i === 0) { const p = project(a[i]); d += `M${p[0]},${p[1]}`; first = false; }
      else if (i === 0) continue; // skip duplicate join
      else { const p = project(a[i]); d += `L${p[0]},${p[1]}`; }
    }
  }
  return d + 'Z';
}
...
function geomToPath(geom) {
  ...
  if (geom.type === 'MultiPolygon') {
    return geom.arcs.map(rings => rings.map(ringToPath).join(' ')).join(' ');
  }
  return '';
}

// M49 -> ISO-2 mapping for G20 + a broad set so most countries get tagged.
// (Names below are kept short. Numbers are UN M49.)
const M49_TO_ISO2 = {
  ...
  '826':'GB','834':'TZ','840':'US','850':'VI','854':'BF','858':'UY','860':'UZ','862':'VE','876':'WF',
  '882':'WS','887':'YE','894':'ZM',
  // -99 / placeholders for disputed: skip
};

const features = topo.objects.countries.geometries;
let paths = '';
for (const g of features) {
  if (!g.id || g.id === '-99') continue;
  const iso = M49_TO_ISO2[String(g.id).padStart(3, '0')];
  if (!iso) continue;
  const d = geomToPath(g);
  if (!d) continue;
  const name = (g.properties && g.properties.name) ? g.properties.name.replace(/"/g, '&quot;') : iso;
  paths += `<path data-iso="${iso}" data-name="${name}" d="${d}"/>\n`;
}

const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 500" preserveAspectRatio="xMidYMid meet">
<g id="countries">
${paths}</g>
</svg>`;

fs.writeFileSync(path.join(ROOT, 'world-map.svg'), svg);
console.log('Wrote world-map.svg', svg.length, 'bytes;', features.length, 'features ->', (svg.match(/<path/g) || []).length, 'paths');
