...
function styleBlock() {
  ...
  <style>
    ...
    .optb-filters{display:flex;justify-content:flex-end;gap:4px;padding:4px 4px 8px;color:#b8b8b8;font-size:13px}
    .optb-filter{cursor:pointer;padding:5px 12px;border-radius:14px;user-select:none;line-height:18px;font:13px Roboto,sans-serif;color:#b8b8b8}
    .optb-filter:hover{color:#dbdbdb}
    ...
    .optb-list{display:flex;flex-direction:column;gap:8px;overflow-y:auto;max-height:calc(100vh - 220px);padding-right:4px}
    .optb-list::-webkit-scrollbar{width:6px}
    .optb-list::-webkit-scrollbar-thumb{background:#3d3d3d;border-radius:3px}
    .optb-card{border:1px solid #4a4a4a;border-radius:8px;background:transparent;cursor:pointer;transition:border-color .15s;padding:0.8px}
    .optb-card:hover{border-color:#8c8c8c}
    ...
    .optb-card-cta{margin-top:12px;display:inline-flex;align-items:center;justify-content:center;background:#fff;color:#0f0f0f;border:1px solid #fff;font:400 14px/18px Roboto,sans-serif;padding:4px 12px;border-radius:6px;cursor:pointer;height:28px}
    ...
    .optb-toolbar{display:flex;align-items:flex-start;gap:8px;padding:4px 0 0;flex-wrap:wrap}
    ...
    .optb-size{display:inline-flex;align-items:center;border:1px solid #4a4a4a;border-radius:4px;background:transparent;height:28px;overflow:hidden}
    .optb-size input{width:54px;text-align:left;padding:0 8px;background:transparent;border:0;color:#dbdbdb;font:400 13px/18px Roboto,sans-serif;outline:none}
    .optb-size button{width:22px;height:22px;border:0;background:transparent;color:#dbdbdb;cursor:pointer;font-size:14px;display:inline-flex;align-items:center;justify-content:center}
    ...
    .optb-iconbar{display:flex;gap:8px;padding:8px 0 12px}
    .optb-iconbtn{width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;color:#b8b8b8;cursor:pointer;background:transparent;border:0;border-radius:4px}
    .optb-iconbtn:hover{color:#dbdbdb;background:rgba(184,184,184,0.08)}

    /* Chart — 750x532. Two y-axis scales, x-axis bottom. */
    .optb-chart{background:transparent;border:0;border-radius:4px;padding:0;position:relative}
    .optb-chart-legend{display:flex;justify-content:center;gap:18px;font:400 12px/16px Roboto,sans-serif;color:#b8b8b8;padding:8px 0}
    ...
    /* Stats row — 54px tall, items 150w (166w with gap) */
    .optb-stats-wrap{position:relative;border-top:1px solid #2e2e2e;padding-top:0}
    .optb-stats{display:flex;align-items:flex-start;gap:16px;padding:8px 0;overflow:hidden}
    ...
    .optb-scroll-r{position:absolute;right:-4px;top:3px;width:48px;height:48px;border-radius:50%;background:rgba(15,15,15,0.92);border:1px solid #3d3d3d;color:#dbdbdb;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:18px;box-shadow:0 0 10px rgba(0,0,0,.45)}
  </style>`;
}
...
function renderChart() {
  ...
  // Left Y: 650..-50 (15 ticks every 50). Right Y: 1.20..-0.10 (14 ticks every 0.10).
  // X: 6890,7035,7180,7325,7470,7615,7760,7910,8060,8225 — strike at 7615.
  ...
  const stepY = plotH / 14; // 14 intervals on left scale (15 ticks)
  const stepRy = plotH / 13; // 13 intervals on right scale (14 ticks)
  const stepX = plotW / 9;   // 9 intervals on x (10 ticks)
  const baseline = top + 13 * stepY; // y where left==0
  ...
  const strikeX = left + 5*stepX; // 7615 = 6th tick (index 5)
  const lossY = baseline + 14;     // -50 line ~ visualization position

  return `
    <div class="optb-chart">
      <svg viewBox="0 0 750 460" width="100%" preserveAspectRatio="none" style="display:block;height:auto;aspect-ratio:750/460">
        ...
        <!-- payoff line cyan/teal: flat at 0 from left to strike, then rises to upper right -->
        ...
        <!-- blue dashed S-curve (azure/58) — option price-like curve flat then rising -->
        ...
        <!-- delta orange dashed S-curve (#fb8c00) — sigmoid through strike -->
        ...
        <!-- breakeven dot at strike intersection -->
...
function renderStats() {
  // 11 stats per Figma. First 5 visible at default width; rest scroll horizontally behind the right scroll button.
  // Values per Figma: Precio subyacente 7.531,25 USD, Beneficio máximo ∞, Pérdida máxima −77,00,
  //   Porcentaje de acierto 34.47%, Punto de equilibrio 7.607, Tamaño del lote 1,
  //   Delta 0,51, Gamma 0,0021, Theta −2,64, Vega 6,02, Rho 1,49
  const items = [
  ...
  function render() {
    ...
    mount.querySelectorAll('.optb-card').forEach(el => {
      el.addEventListener('click', (e) => {
        if (e.target.classList.contains('optb-card-cta')) return;
