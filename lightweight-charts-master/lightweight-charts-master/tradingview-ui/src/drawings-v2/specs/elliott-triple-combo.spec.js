...
export default {
  ...
  label: 'Onda de Elliott — Triple combinación W-X-Y-X-Z',
  icon: 'WXYZ',
  category: 'elliott',
  pointsRequired: 6,
  behaviors: [
  ...
  schema: {
    ...
    especifico: [
      { key: 'showLabels', type: 'toggle', label: 'Mostrar etiquetas W-X-Y-X-Z' },
  ...
  render(g, ctx) {
    ...
    if (opts.validateRules && this.points.length >= 6) {
      ...
      const dX1 = prices[2] - prices[1];
      const dY = prices[3] - prices[2];
      const dX2 = prices[4] - prices[3];
      const dZ = prices[5] - prices[4];
      if (Math.sign(dX1) === Math.sign(dW)) { valid = false; reason = 'X1 no retrocede'; }
      else if (Math.sign(dY) !== Math.sign(dW)) { valid = false; reason = 'Y no continúa'; }
      else if (Math.sign(dX2) === Math.sign(dY)) { valid = false; reason = 'X2 no retrocede'; }
      else if (Math.sign(dZ) !== Math.sign(dW)) { valid = false; reason = 'Z no continúa'; }
    }
