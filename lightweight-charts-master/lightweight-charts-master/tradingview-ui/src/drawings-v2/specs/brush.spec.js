...
export default {
  ...
  schema: {
    estilo: [
      ...
      { key: 'width', type: 'slider', label: 'Grosor', min: 1, max: 12, step: 1 },
      { key: 'opacity', type: 'slider', label: 'Opacidad', min: 0.1, max: 1, step: 0.05 },
    ],
    especifico: [
      { key: 'smoothing', type: 'toggle', label: 'Suavizado (Catmull-Rom)' },
    ],
    coord: 'auto',
  },
  ...
  render(g, ctx) {
    ...
    if (opts.opacity != null && opts.opacity !== 1) path.setAttribute('opacity', opts.opacity);
    g.appendChild(path);
  },
  ...
  hitTest(x, y, ctx) {
    ...
    const tol = Math.max(6, (this.options.width || 3) / 2 + 4);
  ...
  getHandles(ctx) {
    if (this.points.length < 2) return [];
    // Only first and last as handles (free-brush has too many points)
