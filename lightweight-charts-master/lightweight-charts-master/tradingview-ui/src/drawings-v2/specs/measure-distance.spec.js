...
// Measure — TradingView-style ephemeral ruler with Δprice, Δ%, Δtime and bar count.
export default {
  ...
  render(g, ctx) {
    ...
    const nBars = countBars(this, p1.time, p2.time);
    const lines = [];
    ...
    if (opts.showBars && nBars != null) lines.push(`${nBars} bars`);
    if (lines.length) {
