...
// ---------------------------------------------------------------------------
// Arrow up ▲ — single anchor triangular marker, useful for buy signals.
// ---------------------------------------------------------------------------
...
export default {
  ...
  render(g, ctx) {
    ...
    const s = opts.size;
    // Tip sits at (x, y - offsetY); base extends below.
    ...
    if (opts.showLabel && opts.labelText) {
      ...
      t.setAttribute('x', x);
      t.setAttribute('y', baseY + opts.fontSize + 2);
    ...
    this._bbox = { x: x - half - 2, y: tipY - 2, w: half * 2 + 4, h: s + (opts.showLabel ? opts.fontSize + 6 : 4) };
  },
