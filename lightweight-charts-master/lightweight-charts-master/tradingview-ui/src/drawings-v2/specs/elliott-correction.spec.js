...
export default {
  ...
  label: 'Onda de Elliott — Corrección ABC',
  icon: 'ABC',
  category: 'elliott',
  pointsRequired: 4,
  behaviors: [
  ...
  render(g, ctx) {
    ...
    if (opts.validateRules && this.points.length >= 4) {
      ...
      const direction = prices[1] >= prices[0] ? -1 : 1; // A in opposite direction of trend
      // B should retrace A but not exceed origin
      const aDir = prices[1] - prices[0];
      const bDir = prices[2] - prices[1];
      if (Math.sign(aDir) === Math.sign(bDir)) { valid = false; reason = 'B no retrocede'; }
      const cDir = prices[3] - prices[2];
      if (Math.sign(cDir) === Math.sign(bDir)) { valid = false; reason = 'C no continúa'; }
    }
