...
export default {
  ...
  render(g, ctx) {
    ...
    if (opts.extendLeft || opts.extendRight) {
      g.appendChild(Primitives.extendedLine({
        a: p1, b: p2,
        viewBox: { w: ctx.width, h: ctx.height },
        stroke: opts.color, width: opts.width, dashArray: Primitives.dashFromStyle(opts.style),
        leftCap: !opts.extendLeft, rightCap: !opts.extendRight,
      }));
    } else {
