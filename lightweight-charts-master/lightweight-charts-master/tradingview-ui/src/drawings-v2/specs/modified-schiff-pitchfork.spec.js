// drawings-v2/specs/modified-schiff-pitchfork.spec.js
// Modified Schiff Pitchfork — origin is the midpoint of P1,P2 in x only,
// while keeping the y at P1's price. Median runs through midpoint(P2,P3).
import { Primitives } from '../core/primitives.js';

export default {
  type: 'modified-schiff-pitchfork',
  label: 'Horquilla de Schiff modificada',
  icon: 'YSm',
  category: 'channels',
  pointsRequired: 3,
  behaviors: [
  ...
  render(g, ctx) {
    ...
    // Modified Schiff: origin x = midpoint(P1,P2).x, y = P1.y
    const origin = { x: (a.x + b.x) / 2, y: a.y };
    ...
    // Show the modified origin (helps the user see the rule)
    g.appendChild(Primitives.circle({
      cx: origin.x, cy: origin.y, r: 3,
      fill: '#fff', stroke: opts.color, width: 1.5,
    }));
    // Dotted vertical hint linking modified origin to actual midpoint(P1,P2)
    const midP1P2 = { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 };
    g.appendChild(Primitives.line({
      x1: origin.x, y1: origin.y, x2: midP1P2.x, y2: midP1P2.y,
